...
class Penjualan extends MX_Controller {
	...
	public function index($q = 'na')
	{
		...
		$this->grid->init(array (
			...
			'uri_segment'	=> 5,
			'num_rows'		=> $this->penjualan->num_rows($q),
			'items'			=> array (
		...
		$p = $this->grid->params();
		$this->grid->source($this->penjualan->get($p, $q));
		...
		$this->page->view('laporan_penjualan_index', array (
			'q'			=> $qd,
			'grid'		=> $this->grid->draw(),
...
/* Location: ./application/modules/laporan/controllers/Penjualan.php */